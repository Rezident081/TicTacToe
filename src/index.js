class Playground {
  ...
  generateBoard() {
    ...
    for (let index = 0, len = this.board.flat().length; index < len; index++) {
      ...
      div.addEventListener("click", (event) =>
        this.clickItemHandle(event, column, row)
      );
      div.addEventListener("mouseenter", (event) =>
        this.mouseEnterHandle(event, column, row)
      );
      div.addEventListener("mouseout", (event) =>
        this.mouseLeaveHandle(event, column, row)
      );
  ...
  updateSelectedItemClassNames(target) {
    ...
    target.classList.remove(
      `hover-item-${this.activePlayer.symbol.toLowerCase()}`
    );
  }
  ...
  setWinner(player) {
    ...
    this.winnerRef.innerHTML = player
      ? `The winner is: ${player.name}, symbol - ${player.symbol}`
      : "Draw";
...
game.initPlayers(
  new Player({ symbol: "X", name: "Max" }),
  new Player({ symbol: "O", name: "Olga" })
);
game.start();
